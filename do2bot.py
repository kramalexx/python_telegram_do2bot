...
from telegram.ext import (Updater, CommandHandler, MessageHandler, RegexHandler, CallbackQueryHandler,
                          ConversationHandler, InlineQueryHandler, ChosenInlineResultHandler, Filters)
        ...
        try:
            ...
            if userid not in todolist.coworkers:
                if userid == todolist.owner:
                    ...
                    user_data['list'], user_data['current'] = code, message.reply_text(str(todolist),
                                                                                       parse_mode='Markdown',
                                                                                       disable_web_page_preview=True,
                                                                                       reply_markup=createKeyboard(
                                                                                           todolist, userid)).message_id
                ...
                user_data['list'], user_data['current'] = code, message.reply_text(str(todolist), parse_mode='Markdown',
                                                                                   disable_web_page_preview=True,
                                                                                   reply_markup=createKeyboard(todolist,
                                                                                                               userid)).message_id
                bot.edit_message_text(chat_id=message.chat_id, message_id=dbFuncs.getSpecificMessage(code, userid)[0],
                                      text="↓")
...
def setName(update, context):
    ...
    user_data['list'], user_data['current'] = newList.id, message.reply_text(str(newList), parse_mode="Markdown",
                                                                             disable_web_page_preview=True,
                                                                             reply_markup=createKeyboard(newList,
                                                                                                         newList.owner.id)).message_id
    ...
    if not dbFuncs.getAdminTerminal(todolist.id):
        ...
        try:
            if helpFuncs.isInt(ownermsg):
                bot.edit_message_text(chat_id=todolist.owner.id, message_id=ownermsg, text=msgtext,
                                      parse_mode='Markdown', disable_web_page_preview=True, reply_markup=(
                        createKeyboard(todolist, todolist.owner.id) if msgtext != "Closed" else None))
            else:
            ...
            try:
                if helpFuncs.isInt(coworker[1]):
                    bot.edit_message_text(chat_id=coworker[0], message_id=coworker[1], text=msgtext,
                                          parse_mode='Markdown', disable_web_page_preview=True, reply_markup=(
                            createKeyboard(todolist, coworker[0]) if msgtext != "Closed" else None))
                else:
        ...
        for inline in inlines:
            try:
                bot.edit_message_text(inline_message_id=inline[1], text=inlinetext, parse_mode='Markdown',
                                      disable_web_page_preview=True,
                                      reply_markup=(createKeyboard(todolist, -1) if msgtext != "Closed" else None))
            except BadRequest as error:
...
def pushInline(update, context):
    ...
    if action[1] == ListFooter["Check"]:
        ...
        bot.edit_message_text(chat_id=userid, message_id=dbFuncs.getSpecificMessage(action[0], userid)[0],
                              text=_("revoked"))
        ...
        if query.inline_message_id:
            bot.edit_message_reply_markup(inline_message_id=query.inline_message_id,
                                          reply_markup=createAdminKeyboard(action[0], userid))
        else:
            bot.edit_message_reply_markup(chat_id=userid, message_id=query.message.message_id,
                                          reply_markup=createAdminKeyboard(action[0], userid))
...
def pushAdmin(update, context):
    ...
    if action[1] == Options[OptionsOrder[0]]:
        ...
        bot.answer_callback_query(callback_query_id=query.id,
                                  text=_("listaccess").format(_("open") if dbFuncs.isOpen(action[0]) else _("closed")))
        if helpFuncs.isInt(msgid):
            bot.edit_message_reply_markup(chat_id=userid, message_id=msgid,
                                          reply_markup=createAdminKeyboard(action[0], userid))
        else:
        ...
        if helpFuncs.isInt(msgid):
            bot.edit_message_reply_markup(chat_id=userid, message_id=msgid,
                                          reply_markup=createOldKeyboard(action[0], userid))
        else:
    ...
    for ownlist in ownLists:
        ...
        resultList.append(InlineQueryResultArticle(id=temp.id, title=temp.name, description=temp.id,
                                                   ...
                                                   input_message_content=InputTextMessageContent(
                                                       message_text=str(temp) + " `{0}`".format(user_data['tester']),
...
def createKeyboard(todolist, user):
    ...
    for item in items:
        ...
        keyboard.append([InlineKeyboardButton(
            ...
            callback_data=u"{0}_{1}_{2}".format(code, ListFooter["Check"], item.id))])
        for subitem in item.subitems:
            ...
            if subitem.done == True:
                temp2 = "✅"
            subtemp += temp2
            keyboard.append([InlineKeyboardButton(text="{0} {1}{2}".format(subtemp, subitem.name[:249], ''.join(
                ["⠀" for _ in range(249 - len(subitem.name))])),
                                                  callback_data=u"{0}_{1}_{2}".format(code, ListFooter["CheckSub"],
                                                                                      subitem.id))])
    if todolist.owner == user:
        ...
        keyboard.append([InlineKeyboardButton(text=temp, callback_data=u"{0}_{1}".format(code, ListFooter["Remove"])),
                         InlineKeyboardButton(text="{0}".format(len(dbFuncs.getInlineMessages(code))),
    ...
    if len(keyboard) == 0:
        keyboard.append([InlineKeyboardButton(text="➕", url="https://telegram.me/odxbo?start={0}".format(code))])
...
def createOldKeyboard(code, user, page=0):
    ...
    for item in items:
        ...
        if item[3] == True:
            ...
            temp = "✅"
        keyboard.append([InlineKeyboardButton(
            text="{0} {1}{2}".format(temp, item[2][:250], ''.join(["⠀" for _ in range(250 - len(item[2]))])),
            callback_data=u"{0}_{1}_{2}".format(code, ListFooter["Check"], item[0]))])
        if dbFuncs.hasSubItems(item[0]):
            ...
            for subitem in subitems:
                ...
                if subitem[3] == True:
                    temp2 = "✅"
                subtemp += temp2
                keyboard.append([InlineKeyboardButton(text="{0} {1}{2}".format(subtemp, subitem[2][:249], ''.join(
                    ["⠀" for _ in range(249 - len(subitem[2]))])),
                                                      callback_data=u"{0}_{1}_{2}".format(code, ListFooter["CheckSub"],
                                                                                          subitem[0]))])
...
def createAdminKeyboard(code, userid):
    keyboard = [[]]
    k = 2
    keyboard[-1].append(InlineKeyboardButton(text=OptionsOrder[0].format("👥" if dbFuncs.isOpen(code) else "👤"),
                                             callback_data=u"{0}_{1}".format(code, Options[OptionsOrder[0]])))
...
def main(updater):
    ...
    newList = ConversationHandler(
        entry_points=[CommandHandler('new', new, Filters.private),
    ...
    listHandler = ConversationHandler(
        ...
        fallbacks=[CallbackQueryHandler(pushInline),
                   CommandHandler('cancel', cancel, Filters.private, pass_user_data=True)],
        per_message=True
    )
    ...
    dispatcher.add_handler(
        MessageHandler(Filters.private & Filters.text & Filters.reply & (~Filters.update.edited_message), rcvReply))
    dispatcher.add_handler(
        MessageHandler(Filters.text & Filters.private & (~Filters.update.edited_message), rcvMessage))
...
if __name__ == '__main__':
    with open("bottoken.json", "r") as file:
      token = jload(file)['bot_token']
    main(Updater(token, use_context=True))
